...
const DeepDiveGameScreen = ({ setSelectedDeepDiveScreen, }) => {
  ...
  const [selectedDeepGameType, setSelectedDeepGameType] = useState('');
        ...
        <>
          ...
          <View style={{
            ...
            {['Quick Catch', 'Deep Drift', 'Endless Voyage'].map((item, index) => (
              <TouchableOpacity
                onPress={() => {
                  if (selectedDeepGameType === item)
                    setSelectedDeepGameType('');
                  else setSelectedDeepGameType(item);
                }}
                key={item}
                style={[styles.gradientButtonsStyles, {
